...
export function AiFindingCard({ finding, harnessId, onChange }: { finding: AiFinding; harnessId: string; onChange: () => void }) {
  ...
  const reword = async () => { const result = await api.aiReword(finding.id, harnessId); setBody(result.body); setEditing(true); };
  ...
  return <article className="ai-guidance" data-testid={`ai/finding/${finding.id}`}>
    ...
    <div className="ai-actions">
      ...
      <button className="btn sm ghost" onClick={reword}>Reword</button>
...
export function AiQuestionComposer({ target, onCancel, onSent }: { target: AiAnchor; onCancel: () => void; onSent: () => void }) {
  ...
  return <section className="ai-question" aria-label={`Private Q&A about ${target.path}:${target.line}`}>
    <label>Private question<textarea aria-label="Private question" value={question} onChange={event => setQuestion(event.target.value)} /></label>
    {error && <p role="alert" className="ai-error">{error}</p>}
    <div className="ai-actions"><button className="btn sm primary" aria-label="Send private question" disabled={busy || !question.trim()} onClick={submit}>{busy ? 'Asking…' : 'Ask privately'}</button><button className="btn sm ghost" onClick={onCancel}>Cancel</button></div>
  </section>;
}
