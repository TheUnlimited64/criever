...
- **States**: no harness (configure guidance), ready, running review, asking, inline error, general chat empty/populated, contextual summaries empty/populated. A running review has an animated status row in the rail; a pending chat has a visible assistant message placeholder; controls prevent duplicate submissions without hiding previously typed questions.
...
- **Diff line AI action**: permanently visible, individually labelled "AI" button on every actionable row in both unified and split tables. It opens an inline private-question composer; clicking the separate left gutter area still opens the normal comment composer. It cannot be hidden behind a hover `+` decoration.
